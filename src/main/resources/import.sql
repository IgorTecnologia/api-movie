...
INSERT INTO tb_movie (id, title, sub_Title, year_Of_Release, img_Url, synopsis, genre_id) VALUES (UUID(), 'O Garfild', 'O gato sapéca', 2012, 'www.img/url.com', 'Um gato para lá de esperto', @genre2_id);

SET @movie1_id(SELECT id FROM tb_movie WHERE title = 'O homem de ferro');
SET @movie2_id(SELECT id FROM tb_movie WHERE title = 'O Garfild');
